...
#
#     base_model = AutoModelForCausalLM.from_pretrained(path, device_map="cpu",
...
def hf_llm_rephrase(base_model, tokenizer, user_input, system_prompt):
    ...
    state: str
    '''
    #instruction = user_input.split('# AXTree observation at the current time step')[0]
    #raw_observation = user_input.split('# AXTree observation at the current time step')[1]
    #obs_len = len(tokenizer.encode(raw_observation))
    #if obs_len < 1000000:
    chat = [
...
# import os
# os.environ["HF_NO_ALLOCATOR_WARMUP"] = "1"
# model, tokenizers = load_hfmodel(None)
# us = "What is einstein's biggest acheivement ?"
# sys_prompt = "You are a friendly agent responds well in physics questions"
# res = hf_llm_rephrase(model, tokenizers, us, sys_prompt)
# print(res)
