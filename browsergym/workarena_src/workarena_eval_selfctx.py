...
does not support vision. Disabling use_screenshot."""
    ...
    def get_action(self, obs, trial):
        ...
        history = '# ' + splits[2]
        action_space = '# ' + splits[3]
        abstract_examples = '# ' + splits[4]
        concrete_examples = '# ' + splits[5]
...
if __name__ == '__main__':
    ...
    chat_model_args = ChatModelArgs(
                model_name=args.backbone,#'openai/gpt-4o-2024-05-13',#'googleai/gemini-1.5-flash-002',
    ...
    if not os.path.exists(f'workarena_results/{args.backbone.split("/")[-1]}_rephrase/'):
        os.makedirs(f'workarena_results/{args.backbone.split("/")[-1]}_rephrase/')
    ...
    for task, task_id, task_seed in tqdm(tasks):
        try:
            ...
            with open(f'workarena_results/{args.backbone.split("/")[-1]}_rephrase/{task_id}_{task_seed}.json', 'w') as f:
                json.dump(rollout_dict, f, indent=4)
            print('Rewards: ', rewards)
        except Exception as e:
            print(e)
            rollout_dict = {'task_idx': f"{task_id}_{task_seed}", 'reward': 'NA', 'rollout': rollout}
            with open(f'workarena_results/{args.backbone.split("/")[-1]}_rephrase/{task_id}_{task_seed}.json', 'w') as f:
                json.dump(rollout_dict, f, indent=4)
            rewards.append('NA')
            print('Rewards: ', rewards)
