...
does not support vision. Disabling use_screenshot."""
    ...
    def get_action(self, obs, trial):
        ...
        ######## 2. get reasoning from the reasoner LM ################ TODO: fix the code using rephraser_lm and rephrase_tok
...
if __name__ == '__main__':
    ...
    rephrase_lm, rephrase_tok = load_hfmodel(f"ckpt/workarena_sft_iter_{args.iter}/checkpoint-{args.ckpt_step}")
    ...
    for task, task_id, task_seed in tqdm(tasks):
        try:
            ...
            env = gym.make(os.path.join('browsergym', task_id), 
                        headless=True,
                        )
            ############## reset environment #############################
            ...
            if reward == 1.0:
                ...
                with open(f'workarena_results/{args.action_backbone.split("/")[-1]}_iter{args.iter}/{task_id}_{task_seed}.json', 'w') as f:
