...
class WebAgentSiteEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if action_name == 'search':
            try:
                search_bar = self.browser.find_element(by=By.ID, value='search_input')
            except Exception:
                pass
            else:
    ...
    def get_available_actions(self):
        ...
        buttons = self.browser.find_elements(by=By.CLASS_NAME, value='btn')

        # product_links = self.browser.find_elements_by_class_name('product-link')
        try:
            product_links = self.browser.find_elements(by=By.CLASS_NAME, value='product-link')
        except Exception as ex:
            print(ex)
            product_links = None
        # buying_options = self.browser.find_elements_by_css_selector("input[type='radio']")
        try:
            buying_options = self.browser.find_elements(by=By.CSS_SELECTOR, value="input[type='radio']")
        except Exception as ex:
            print(ex)
            buying_options = []
